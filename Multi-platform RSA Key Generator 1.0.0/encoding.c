...
void shiftLeftByN(BigInteger *x, int n)
{
    int SHR_BY = 64 - n, i = x->size++;
    uint64 MSB_N = (uint64)-1 << SHR_BY;

    for (x->uints[i] = (x->uints[i - 1] & MSB_N) >> SHR_BY, --i; i; --i)
        x->uints[i] = x->uints[i] << n | (x->uints[i - 1] & MSB_N) >> SHR_BY;
...
BigInteger randomBigInteger(int d, uint32 s)
{
    int n = bitLength(RAND_MAX), i = d / n + 1, size = n * (i + 1) / 64 + 2;
    uint64 *heap = malloc8(2 * size);
    BigInteger t, p, r;
    t.uints = heap, p.uints = &heap[size], r.uints = malloc8(size);

    for (srand(s), one(&p), zero(&r); i; --i)
        multiplyDigit(p, rand(), &t), add(&r, t), shiftLeftByN(&p, n);

    bitwiseMod(&r, d), free(heap);

    return r;
}

BigInteger seededBigInteger(int d, Seeds seeds)
{
    BigInteger t = randomBigInteger(d, seeds.uints[1]), r;
    lockSize(&t, d), r.uints = malloc8(t.size + 2), zero(&r);
    int i;

    for (i = 1; i != seeds.size;
         free(t.uints), t = randomBigInteger(d, seeds.uints[++i]))
        if (i & 1)
            add(&r, t);
        else
            _xor(r, t);

    bitwiseMod(&r, d), free(t.uints);
...
Bytes PUBLIC_HEADER(void)
{
    Bytes r;
    r.size = 15, r.bytes = malloc1(r.size);
...
Bytes PRIVATE_HEADER(void)
{
    Bytes r;
    r.size = 18, r.bytes = malloc1(r.size);
...
Bytes getSizePrefix(int size)
{
    ...
    if (size <= 0x7F)
        r.size = 1, r.bytes = malloc1(r.size),
        r.bytes[0] = size;
    else if (size <= 0xFF)
        r.size = 2, r.bytes = malloc1(r.size),
        r.bytes[0] = 0x81, r.bytes[1] = size;
    else if (size <= 0xFFFF)
        r.size = 3, r.bytes = malloc1(r.size),
...
Bytes bytesToASN1Int(Bytes b)
{
    ...
    if (b.bytes[b.size - 1] & B7)
        tc.size = b.size + 1,
        tc.bytes = malloc1(tc.size), tc.bytes[i++] = 0;
    else
        tc.size = b.size,
        tc.bytes = malloc1(tc.size);

    for (ts = getSizePrefix(tc.size), j = b.size - 1; j != -1; --j)
        tc.bytes[i++] = b.bytes[j];

    r.size = ts.size + tc.size + 1, r.bytes = malloc1(r.size);
...
Bytes buildPublicPKCS1(Bytes n, Bytes e)
{
    ...
    r.size = ts.size + size + 1, r.bytes = malloc1(r.size);
...
Bytes buildPublicPKCS8(Bytes b)
{
    ...
    r.size = i, r.bytes = malloc1(r.size), i = 0;
    r.bytes[i++] = 0x30;
...
{
    Bytes r;
    r.size = 8 * b.size, r.bytes = malloc1(r.size);
...
void createRSAKeys(int size, Seeds seeds,
                   const char *pubName1, const char *pubName8,
                   const char *priName1, const char *priName8)
{
    time_t start = clock();
    BigInteger p = seededBigInteger(size / 2, seeds),
               n = randomBigInteger(size, seeds.uints[0]),
    ...
    dp.uints = &heap1[i], i += dtSize, m.uints = &heap1[i++], random64(&m);
    ...
    if (_eq(m, m_))
    {
        ...
        printf("KEYS NOT VALID - Try again with different seeds."),
